...
namespace CompuMarket_App
{
    ...
    {
        private readonly Services _currentProduct = new Services();
        public ProductEditPage(Services selectedProduct)
        {
            InitializeComponent();
            if (selectedProduct != null)
                _currentProduct = selectedProduct;
            DataContext = _currentProduct;
            if (_currentProduct.UserId == 0)
        ...
        {
            if (_currentProduct.UserId == 0)
                IlyasovaEntities.GetContext().Services.Add(_currentProduct);
        ...
        private void DeleteClick_Click(object sender, RoutedEventArgs e)
        {
                ...
                MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
            {
                try
                {
                    IlyasovaEntities.GetContext().Services.Remove(_currentProduct);
                    IlyasovaEntities.GetContext().SaveChanges();
